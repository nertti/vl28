...
?>
<?php
$elementInfo = $GLOBALS['CATALOG_CURRENT_ELEMENT_INFO'] ?? [];

$crossSalesIds = !empty($elementInfo['CROSS_SALES_IDS']) ? $elementInfo['CROSS_SALES_IDS'] : [];
$currentSectionId = !empty($elementInfo['CURRENT_SECTION_ID']) ? $elementInfo['CURRENT_SECTION_ID'] : 0;
$currentElementId = !empty($elementInfo['CURRENT_ELEMENT_ID']) ? $elementInfo['CURRENT_ELEMENT_ID'] : 0;

// Формируем фильтры для catalog.section
$GLOBALS['CROSS_SALES_IDS'] = $crossSalesIds;
$GLOBALS['CURRENT_SECTION_ID'] = $currentSectionId;
...
<section class="products products_others">
    ...
    <?php $APPLICATION->IncludeComponent(
        ...
        array(
            ...
            "BROWSER_TITLE" => "-",
            "CACHE_FILTER" => "Y", // ИЗМЕНЕНО: Включаем кэширование фильтра
            "CACHE_GROUPS" => "Y",
            "CACHE_TIME" => "36000000",
            "CACHE_TYPE" => "Y",
            "COMPATIBLE_MODE" => "Y", // ИЗМЕНЕНО: Отключаем совместимость старых версий (ускоряет работу)
            "CONVERT_CURRENCY" => "Y",
            "CURRENCY_ID" => "RUB",
            ...
            "DISPLAY_BOTTOM_PAGER" => "N", // ИЗМЕНЕНО: В рекомендациях пагинация обычно не нужна
            "DISPLAY_TOP_PAGER" => "N",
            "ELEMENT_SORT_FIELD" => "sort", // Добавлено дефолтное поле сортировки
            ...
            "HIDE_NOT_AVAILABLE" => "Y", // ИЗМЕНЕНО: Скрываем отсутствующие товары в блоке рекомендаций
            ...
            "OFFERS_LIMIT" => "1", // ИЗМЕНЕНО: Снижаем лимит торговых предложений на один товар
            ...
            "SEF_MODE" => "Y",
            "SET_BROWSER_TITLE" => "N", // ИЗМЕНЕНО: Запрещаем ломать SEO страницы товара
            "SET_LAST_MODIFIED" => "N",
            "SET_META_DESCRIPTION" => "N", // ИЗМЕНЕНО: Запрещаем ломать SEO страницы товара
            "SET_META_KEYWORDS" => "N", // ИЗМЕНЕНО: Запрещаем ломать SEO страницы товара
            "SET_STATUS_404" => "N",
            "SET_TITLE" => "N", // ИЗМЕНЕНО: Запрещаем менять основной заголовок H1 товара
            "SHOW_404" => "N",
